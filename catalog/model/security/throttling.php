...
class Throttling extends \Opencart\System\Engine\Model {
  ...
  public function isIpAllowed($key) {
    $hourly_limit = $this->config->has('throttling_' . $key . '_ip_per_hour') ? $this->config->get('throttling_' . $key . '_ip_per_hour') : $this->config->get('throttling_default_ip_per_hour');
    $daily_limit = $this->config->has('throttling_' . $key . '_ip_per_day') ? $this->config->get('throttling_' . $key . '_ip_per_day') : $this->config->get('throttling_default_ip_per_day');
  ...
  public function isDataAllowed($key, $data) {
    $hourly_limit = $this->config->has('throttling_' . $key . '_data_per_hour') ? $this->config->get('throttling_' . $key . '_data_per_hour') : $this->config->get('throttling_default_data_per_hour');
    $daily_limit = $this->config->has('throttling_' . $key . '_data_per_day') ? $this->config->get('throttling_' . $key . '_data_per_day') : $this->config->get('throttling_default_data_per_day');
